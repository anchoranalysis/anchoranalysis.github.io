...
| -tr | Suggests an <span class="optionArg">image dimensions to resize to</span> (e.g. `1024x768`) or a <span class="optionArg">scaling factor</span> (e.g.`0.5`)<br>• To resize preserving aspect ratio: `200x` or `x50` to a particular width/height, and `1000x500+` maximally within these dimensions.<br>• The order of dimensions is always `width`x`height`<br>• No scaling in the z-dimension is supported. |
